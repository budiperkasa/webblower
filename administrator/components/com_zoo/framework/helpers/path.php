...
class PathHelper extends AppHelper {
	...
	*/
	public function url($resource) {
	    ...
	    if ($url) {
	        ...
	        $url = JURI::root(true).'/'.ltrim(preg_replace('/'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', JPATH_ROOT), '/i').'/', '', $url, 1), '/');
	    }
	...
	*/
	protected function _find($paths, $file) {
		...
		foreach ($paths as $path) {
			if (($fullpath = realpath("$path/$file")) && stripos($fullpath, $root, 0) === 0) {
				return $fullpath;
			}
		}
	...
	*/
	public function relative($path) {
		...
		return ltrim(preg_replace('/'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', JPATH_ROOT), '/i').'/', '', $path, 1), '/');
	}
