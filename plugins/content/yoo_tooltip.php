...
function plgContentYOOtooltipReplace(&$row, &$matches, $count, $regex, $params) {
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if ($tooltip_text != '') {
			...
			$replace  = '<div id="' . $id . '" class="yoo-tooltip-toggler">' . $title . '</div>';
			$replace .= "<script type=\"text/javascript\">new YOOtooltip('". $id ."', '" . $tooltip_text . "', { mode: '" . $mode . "', display: '" . $display . "', width: " . $width . ", style: '" . $style . "' });</script>";
		}
